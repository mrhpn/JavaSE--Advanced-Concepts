...
public class MapDemo {
	public static void show() {
		...
		// Looping map (2) - entrySet()
		for (var entry : map.entrySet()) {
			System.out.println(entry); // Output: e1=a++ e2=b
			System.out.println(entry.getKey()); // Output: e1 e2
			System.out.println(entry.getValue()); // Output: a++ b
		}
		
		// Looping map (3) - values()
		for (var customer2 : map.values()) // returns collection of Customers
			System.out.println(customer2); // Output: a++ b // DOES NOT Grauntee the order
	}
}
